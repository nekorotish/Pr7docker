...
export const CreatePost = ({navigation}) => {
    ...
    const onSubmit = async (values) => {
        ...
        if (image === null) {
            ...
            // values.tags = values.tags.replace(/,\s+/g, ',').replace(/\s+,/g, ',').replace(/\s+/g, ' ').trim();
            ...
            try {
                ...
                //
                // console.log(fields)
                const dataPost = await axios.post('/postgresql/posts', fields)
    ...
    if (!isAuth) {
        navigation.goBack();
        // alert("Вы не зарегестрировались, не можете создавать посты")
    }
    ...
    return (
        <SafeAreaView>
            <ScrollView>
                ...
                {
                    image === null ? (
                        <AddCard onTouchEndCapture={() => pickImage()}>
